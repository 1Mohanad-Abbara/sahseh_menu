# Agent Handoff: Sahseh Static Menu
...
- Product rows and controls may have desktop mouse hover effects, but should not gain touch/click color flashes or active scale animation. The back-to-top button keeps the same colors on hover and only scales slightly. Keep keyboard `:focus-visible` outlines.
...
Current JS renders categories and products in the array order from JSON. It does not sort by `order` and does not filter by `available`, `visibleInDineIn`, or `visibleInOrdering`. `priceText` wins when present; otherwise a numeric `price` is displayed without unnecessary decimal zeros.
...
- Verify the back-to-top button appears after scrolling, hides near the footer, keeps the same colors on hover in both themes, and scales slightly on desktop hover.
